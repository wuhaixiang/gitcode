...
void USART2_IRQHandler(void)
{
    ...
    rt_interrupt_leave();
#else
    static unsigned char recv_data;//用于接受串口2发送的数据
    extern struct rt_semaphore sem;//利用静态信号量，定义信号量
    extern struct rt_mailbox mb;//利用静态邮箱，定义邮箱
    rt_interrupt_enter();//进入中断
    recv_data==(USART2->DR&0XFF);//接收数据，同时清楚接收中断
